...
public class LeanBeanJpaDao implements LeanBeanDao {
    ...
    public List<Meeting> findMeetingsByDeviceId(long deviceId) {
        List<Meeting> resultList = em.createQuery("select distinct(m) from Meeting m left join m.topics t left join m.votes v where m.device.id = :deviceIdMeeting or t.device.id = :deviceIdTopic or v.device.id = :deviceIdVote order by m.id desc")
                .setParameter("deviceIdMeeting", deviceId)
                .setParameter("deviceIdTopic", deviceId)
                .setParameter("deviceIdVote", deviceId)
                .setMaxResults(10)
                .getResultList();
        return resultList;
    }
...
left join topic t on t.meeting_id = m.id
left join vote v on v.meeting_id = m.id
where m.device_id = 2
or t.device_id = 2
or v.device_id = 2
order by m.id desc
